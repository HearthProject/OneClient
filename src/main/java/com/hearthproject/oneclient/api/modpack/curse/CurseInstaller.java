...
public class CurseInstaller extends ModpackInstaller {
    ...
    public CurseInstaller(Manifest manifest, File pack) {
    ...
    public List<ModInstaller> getMods() {
        ...
        return manifest.files.stream().map(f -> new CurseModInstaller(f.projectID, f.fileID)).collect(Collectors.toList());
    }
}
